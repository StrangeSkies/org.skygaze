...
import uk.co.elionline.emerge.mathematics.expressions.CompoundExpression;
import uk.co.elionline.emerge.mathematics.expressions.ConditionalExpression;
import uk.co.elionline.emerge.mathematics.expressions.Expression;
import uk.co.elionline.emerge.mathematics.expressions.IdentityExpression;
import uk.co.elionline.emerge.mathematics.expressions.collections.ListExpressionView;
import uk.co.elionline.emerge.mathematics.functions.AssignmentOperation;
...
import uk.co.elionline.emerge.mathematics.functions.NumberToValueFunction;
...
import uk.co.elionline.emerge.mathematics.geometry.DimensionalityException;
import uk.co.elionline.emerge.mathematics.logic.BooleanValue;
import uk.co.elionline.emerge.mathematics.values.DoubleArrayListView;
import uk.co.elionline.emerge.mathematics.values.DoubleValue;
import uk.co.elionline.emerge.mathematics.values.DoubleValueFactory;
import uk.co.elionline.emerge.mathematics.values.FloatArrayListView;
import uk.co.elionline.emerge.mathematics.values.FloatValue;
import uk.co.elionline.emerge.mathematics.values.FloatValueFactory;
import uk.co.elionline.emerge.mathematics.values.IntArrayListView;
...
import uk.co.elionline.emerge.mathematics.values.LongArrayListView;
import uk.co.elionline.emerge.mathematics.values.LongValue;
import uk.co.elionline.emerge.mathematics.values.LongValueFactory;
...
import uk.co.elionline.emerge.utilities.collections.NullPointerInCollectionException;
 ...
 */
public abstract class VectorImplementation<S extends VectorImplementation<S, V>, V extends Value<V>>
	...
	private VectorImplementation(Order order, Orientation orientation) {
	...
	public VectorImplementation(int size, Order order, Orientation orientation,
	...
	public VectorImplementation(int size, Factory<V> valueFactory) {
		this(size, getDefaultOrder(), getDefaultOrientation(), valueFactory);
	}

	public VectorImplementation(Orientation orientation, Factory<V> valueFactory,
			Matrix<?, ?> other) {
		this(other.getOrder(), orientation, valueFactory, other.getData2(), null);
	}

	public VectorImplementation(Factory<V> valueFactory, Matrix<?, ?> other) {
		this(getDefaultOrientation(), valueFactory, other);
	}

	public VectorImplementation(Orientation orientation, Matrix<?, V> other) {
		this(other.getOrder(), orientation, other.getData2(), (Void) null);
	}

	public VectorImplementation(Matrix<?, V> other) {
		this(getDefaultOrientation(), other);
	}

	public VectorImplementation(Factory<V> valueFactory, Vector<?, ?> other) {
		this(other.getOrder(), other.getOrientation(), valueFactory, other
				.getData());
	}

	public VectorImplementation(Vector<?, V> other) {
		this(other.getOrder(), other.getOrientation(), other.getData());
	}

	public <I> VectorImplementation(Order order, Orientation orientation,
		...
		for (I value : values) {
			data.add(creationOperation.apply(value));
		}

		finalise();
	}

	public <I> VectorImplementation(List<? extends I> values,
			UnaryOperation<? extends V, ? super I> creationOperation) {
		this(getDefaultOrder(), getDefaultOrientation(), values, creationOperation);
	}

	protected <I> VectorImplementation(Order order, Orientation orientation,
		...
		if (order == orientation.getAssociatedOrder()) {
			try {
				DimensionalityException.checkEquivalence(values.size(), 1);

				List<? extends I> major = values.get(0);

				if (major == null) {
					throw new NullPointerException();
				}

				DimensionalityException.checkValid(major.size());
			...
			for (List<? extends I> major : values) {
				data.add(creationOperation.apply(major.get(0)));
			}
		}
	...
	protected <I> VectorImplementation(List<? extends List<? extends I>> values,
			UnaryOperation<? extends V, ? super I> creationOperation, Void flag) {
		this(getDefaultOrder(), getDefaultOrientation(), values, creationOperation,
				null);
	}

	public VectorImplementation(Order order, Orientation orientation,
			final Factory<V> valueFactory, List<? extends Value<?>> values) {
		this(order, orientation, values, new UnaryOperation<V, Value<?>>() {
			@Override
	...
	public VectorImplementation(Factory<V> valueFactory,
			List<? extends Value<?>> values) {
		this(getDefaultOrder(), getDefaultOrientation(), valueFactory, values);
	}

	protected VectorImplementation(Order order, Orientation orientation,
	...
	protected VectorImplementation(Factory<V> valueFactory,
			List<? extends List<? extends Value<?>>> values, Void flag) {
		this(getDefaultOrder(), getDefaultOrientation(), valueFactory, values, null);
	}

	public VectorImplementation(Order order, Orientation orientation,
			Factory<V> valueFactory, Value<?>... values) {
		this(order, orientation, valueFactory, Arrays.asList(values));
	}

	public VectorImplementation(Factory<V> valueFactory, Value<?>... values) {
		this(getDefaultOrder(), getDefaultOrientation(), valueFactory, values);
	}

	public VectorImplementation(Order order, Orientation orientation,
			List<? extends Value<V>> values) {
		this(order, orientation, values, new UnaryOperation<V, Value<V>>() {
			@Override
	...
	public VectorImplementation(List<? extends Value<V>> values) {
	...
	protected VectorImplementation(
			List<? extends List<? extends Value<V>>> values, Void flag) {
		this(getDefaultOrder(), getDefaultOrientation(), values, (Void) null);
	}

	@SafeVarargs
	public VectorImplementation(Order order, Orientation orientation,
			Value<V>... values) {
		this(order, orientation, Arrays.asList(values));
	}

	@SafeVarargs
	public VectorImplementation(Value<V>... values) {
		this(getDefaultOrder(), getDefaultOrientation(), values);
	}

	public VectorImplementation(final boolean copyByReference, Order order,
			Orientation orientation, List<? extends V> values) {
		this(order, orientation, values, new UnaryOperation<V, V>() {
			@Override
	...
	public VectorImplementation(final boolean copyByReference,
			List<? extends V> values) {
		this(copyByReference, getDefaultOrder(), getDefaultOrientation(), values);
	}

	protected VectorImplementation(final boolean copyByReference, Order order,
	...
	protected VectorImplementation(final boolean copyByReference,
	...
	@SafeVarargs
	public VectorImplementation(boolean copyByReference, Order order,
			Orientation orientation, V... values) {
		this(copyByReference, order, orientation, Arrays.asList(values));
	}

	@SafeVarargs
	public VectorImplementation(boolean copyByReference, V... values) {
		this(copyByReference, getDefaultOrder(), getDefaultOrientation(), values);
	}
	...
	public VectorImplementation(Order order, Orientation orientation,
		...
		this(true, order, orientation, new ListTransformationView<>(
				Arrays.asList(values), new NumberToValueFunction<V>(valueFactory)));

	}

	public VectorImplementation(Factory<V> valueFactory, Number... values) {
		this(getDefaultOrder(), getDefaultOrientation(), valueFactory, values);
	}

	public VectorImplementation(Order order, Orientation orientation,
			Factory<V> valueFactory, int[] values) {
		this(true, order, orientation, new IntArrayListView<>(values, valueFactory));
	}

	public VectorImplementation(Factory<V> valueFactory, int[] values) {
		this(getDefaultOrder(), getDefaultOrientation(), valueFactory, values);
	}

	public VectorImplementation(Order order, Orientation orientation,
			Factory<V> valueFactory, long[] values) {
		this(true, order, orientation,
				new LongArrayListView<>(values, valueFactory));
	}

	public VectorImplementation(Factory<V> valueFactory, long[] values) {
		this(getDefaultOrder(), getDefaultOrientation(), valueFactory, values);
	}

	public VectorImplementation(Order order, Orientation orientation,
			Factory<V> valueFactory, float[] values) {
		this(true, order, orientation, new FloatArrayListView<>(values,
				valueFactory));
	}

	public VectorImplementation(Factory<V> valueFactory, float[] values) {
		this(getDefaultOrder(), getDefaultOrientation(), valueFactory, values);
	}

	public VectorImplementation(Order order, Orientation orientation,
			Factory<V> valueFactory, double[] values) {
		this(true, order, orientation, new DoubleArrayListView<>(values,
				valueFactory));
	}

	public VectorImplementation(Factory<V> valueFactory, double[] values) {
		this(getDefaultOrder(), getDefaultOrientation(), valueFactory, values);
	}
	...
	public VectorImplementation(Order order, Orientation orientation,
		...
		this(true, order, orientation, new ListTransformationView<>(
				Arrays.asList(values), new Function<List<V>, int[]>() {
					@Override
	...
	public VectorImplementation(Order order, Orientation orientation,
		...
		this(true, order, orientation, new ListTransformationView<>(
				Arrays.asList(values), new Function<List<V>, long[]>() {
					@Override
	...
	public VectorImplementation(Order order, Orientation orientation,
		...
		this(true, order, orientation, new ListTransformationView<>(
				Arrays.asList(values), new Function<List<V>, float[]>() {
					@Override
	...
	public VectorImplementation(Order order, Orientation orientation,
		...
		this(true, order, orientation, new ListTransformationView<>(
				Arrays.asList(values), new Function<List<V>, double[]>() {
					@Override
	...
	public static void setDefaultOrientation(Orientation defaultOrientation) {
		VectorImplementation.defaultOrientation = defaultOrientation;
	}

	public static Order getDefaultOrder() {
		return MatrixImplementation.getDefaultOrder();
	}

	public static void setDefaultOrder(Order defaultOrder) {
		MatrixImplementation.setDefaultOrder(defaultOrder);
	}
	...
	public Vector2<IntValue> getDimensions2() {
		if (getOrientation() == Orientation.Column) {
			return new Vector2<IntValue>(IntValueFactory.instance(), 1,
					getDimensions());
		} else {
			return new Vector2<IntValue>(IntValueFactory.instance(), getDimensions(),
					1);
		}
	}
	...
	public final VectorN<V> getMajorVector(int index) {
		...
		return new VectorN<V>(true, data);
	}

	@Override
	public final VectorN<V> getMinorVector(int index) {
		return new VectorN<V>(true, data.get(index));
	}
	...
	public final List<VectorN<V>> getMajorVectors() {
		return new AbstractList<VectorN<V>>() {
			@Override
			public VectorN<V> get(int index) {
