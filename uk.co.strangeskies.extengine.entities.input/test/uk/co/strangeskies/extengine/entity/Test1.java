...
public class Test1 {
	...
	public void run() {
		...
		assemblage2.getInitialisers(velocity).add((data, context) -> data
		...
		assemblage1.getInitialisers(parrot).add((data, context) -> {
			try {
				data.set("Child position: " + context.getSubcontext(assemblage1).getData(position) + " num "
