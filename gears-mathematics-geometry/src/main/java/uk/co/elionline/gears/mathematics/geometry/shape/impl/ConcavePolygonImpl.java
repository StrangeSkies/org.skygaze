...
public class ConcavePolygonImpl<V extends Value<V>> extends
	...
	public void sanitize(int toleranceLevel) {
	  ...
	  // snap together points within tolerence squares and remove zero length segments
	  for (List<Vector2<V>> i : vertices.begin(); i < vertices.end() - 1; ++i)
	    for (List<Vector2<V>> j : i + 1; j < vertices.end(); ++j)
	      if (ToleranceEqual((*j).x, (*i).x, tolerance) && ToleranceEqual((*j).y, (*i).y, tolerance))
	        if (j == i + 1)
	          vertices.erase(j--);
	        else {
	          (*j).x = (*i).x;
	  ...
	  if (ToleranceEqual(vertices.front().x, vertices.back().x, tolerance) && ToleranceEqual(vertices.front().y, vertices.back().y, tolerance))
	    vertices.erase(vertices.begin());

	  // snap lines to all points with which they touch the ULP tolerence squares of.
	  ...
	  for (List<Vector2<V>> i : vertices.begin(); i < vertices.end(); ++i) {
	    ...
	    for (List<Vector2<V>> j : vertices.begin(); j < vertices.end(); ++j) {
	       if (j !=  i && j != last) { // && j !touching i or last
	         Box box = Box(Vector2<V>(*j) -= Vector2<V>(tolerance), Vector2<V>(*j) += Vector2<V>(tolerance));

	         if (box.CrossesLine(line)) {
	           float distanceAlongLine = Matrix2(!line.GetAB().Normalize(), *j - line.GetA()).Det();
	    ...
	    for (std::multimap<float, List<Vector2<V>>::iterator> j : pointsAlongLine.begin(); j != pointsAlongLine.end(); ++j) {
	      vertices.insert(i, (*j).first);
	      // record touching points generated ( return value of insert() <|> (*insert).first )
	    }

	    // make sure to perform procedure iteratively by returning to first new subline (if any)
	  ...
	  // SHOULD BE ROBUST IF WE CAN GUARENTEE SOME MAXIMUM ULP IMPRECISION IN INTERSECTION POINTS
